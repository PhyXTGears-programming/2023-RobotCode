...
// 50 ticks per second
#define SECONDS_TO_TICKS(ti) (ti * 50)
...
autoBalance::autoBalance(Drivetrain *drivetrain)
{
    ...
    onChargeStationDegree = DEGREES_TO_RADIANS(11.0);
    ...
    levelDegree = DEGREES_TO_RADIANS(3.0);
    bangBangDegrees = DEGREES_TO_RADIANS(5.0);
...
{ // return in radians
    return std::atan2(c_drivetrain->getYTilt(),
                      c_drivetrain->getFieldHeading()) *
           (180.0 / M_PI);
}

void autoBalance::init(){
    yTiltOffset = c_drivetrain->getYTilt();
}
...
double autoBalance::getTilt()
{
    ...
    //     return -std::sqrt(pitch*pitch + roll*roll);
    // }
    return c_drivetrain->getYTilt() - yTiltOffset;
}
...
{
    frc::SmartDashboard::PutNumber("y ofset", RAD_2_DEG(yTiltOffset));
    frc::SmartDashboard::PutNumber("y tilt processed", RAD_2_DEG(getTilt()));
    frc::SmartDashboard::PutNumber("auto State", state);
    frc::SmartDashboard::PutNumber("debounce", debounceCount);
    switch (state)
    {
